...
function autoContentType (req, file, cb) {
  // FIXME this could be replaced by a fromStream that performs this same feature ( reading a chunk and running the file id from there )
  // fileTypeStream
  // util.callbackify(upload.done.bind(upload))(function (err, result) {

  file.stream.once('data', function (firstChunk) {
    util.callbackify(fromBuffer)(firstChunk, function (err, result) {
      if (err) return cb(err)
      var type = result
      console.log('autoContentType firstChunk length: ' + firstChunk.length)
      var mime = 'application/octet-stream' // default type
      console.log('autoContentType type: ' + type)
...
// The file data will be given to you as a stream (file.stream)
...
S3Storage.prototype._handleFile = function (req, file, cb) {
  collect(this, req, file, function (err, opts) {
    ...
    finalStream.on('end', () => {
      console.log('All done. Size is ' + readBytes)
    })

    params.Body.pipe(finalStream)
    ...
    upload.on('httpUploadProgress', function (ev) {
      currentSize = Math.max(currentSize, ev.loaded)
    })
    ...
    util.callbackify(upload.done.bind(upload))(function (err, result) {
      ...
      console.log('multerS3 currentSize: ', currentSize)
      console.log('multerS3 readBytes: ', readBytes)
      cb(null, {
        size: readBytes, // was currentSize,
        bucket: opts.bucket,
        key: opts.key,
        acl: opts.acl,
        contentType: opts.contentType,
